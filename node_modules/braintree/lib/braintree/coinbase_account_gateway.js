...
CoinbaseAccountGateway = (function(_super) {
  ...
  CoinbaseAccountGateway.prototype.find = function(token, callback) {
    if (token.trim() === '') {
      ...
      return this.gateway.http.get("/payment_methods/coinbase_account/" + token, function(err, response) {
        if (err) {
  ...
  CoinbaseAccountGateway.prototype["delete"] = function(token, callback) {
    return this.gateway.http["delete"]("/payment_methods/coinbase_account/" + token, callback);
  };
